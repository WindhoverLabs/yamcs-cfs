 ...
 * @author nm
 */
public class SerialStreamInOutProvider extends AbstractYamcsService implements Link, StreamSubscriber, SystemParametersProducer, Runnable {
    ...
    protected void processPacket(TmPacket tmpkt) {
        ...
        byte[] trimmedByteArray = Arrays.copyOfRange(byteArray, this.leftTrim, byteArray.length - this.rightTrim);
    ...
    {
        spLinkStatus = sysParamCollector.createEnumeratedSystemParameter(linkName + "/status", Status.class,
                "The current status of this interface");

        spInPacketCount = sysParamCollector.createSystemParameter(linkName + "/packetInCount", Type.UINT64,
                "The total number of packets that have been received via this interface");

        spOutPacketCount = sysParamCollector.createSystemParameter(linkName + "/packetOutCount", Type.UINT64,
                "The total number of packets that have been transmitted via this interface");

        spInPacketRate = sysParamCollector.createSystemParameter(linkName + "/packetInRate", Type.UINT64,
                "The rate packets that have been received via this interface");

        spOutPacketRate = sysParamCollector.createSystemParameter(linkName + "/packetOutRate", Type.UINT64,
                "The rate packets that have been transmitted via this interface");

        spInDataCount = sysParamCollector.createSystemParameter(linkName + "/dataInCount", Type.UINT64,
                "The total number of bytes that have been received via this interface");

        spOutDataCount = sysParamCollector.createSystemParameter(linkName + "/dataOutCount", Type.UINT64,
                "The total number of bytes that have been transmitted via this interface");

        spInDataRate = sysParamCollector.createSystemParameter(linkName + "/dataInRate", Type.UINT64,
                "The bit rate of data that has been received via this interface");

        spOutDataRate = sysParamCollector.createSystemParameter(linkName + "/dataOutRate", Type.UINT64,
    ...
    protected void collectSystemParameters(long time, List<ParameterValue> list) {
    	...
    	list.add(SystemParametersService.getPV(spInPacketRate, time, inPacketRateMeter.getFiveSecondsRate()));
    	list.add(SystemParametersService.getPV(spOutPacketRate, time, outPacketRateMeter.getFiveSecondsRate()));
    	list.add(SystemParametersService.getPV(spInDataRate, time, inDataRateMeter.getFiveSecondsRate()));
    	list.add(SystemParametersService.getPV(spOutDataRate, time, outDataRateMeter.getFiveSecondsRate()));
