 ...
 * when possible. This improves data integrity and reliability when parsing streams that are lossy or when it is not
 * guaranteed to begin at the beginning of a packet. This is a stateful design that will reset back to the initial
 * unsynchronized state when it detects the possibility that its parsing invalid data. For example, if the integrator
 * knows the stream will not contain any packet greater than a certain size, not just the maximum CCSDS size, the
 * maximum size can bet set as a configurable parameter. In this case, when a length parameter is read that exceeds the
 * maximum size, the parser will detect this as an out of sync condition, and reset back to the initial out of sync
 * state.
 * 
...
public class RFC1055PacketInputStream implements PacketInputStream {
    private static final byte END = (byte) 0x0300;
    private static final byte ESC_ESC = (byte) 0x0333;
    private final byte ESC = (byte) 0x0334;
    private final byte ESC_END = (byte) 0x0335;
    ...
    public void init(InputStream inputStream, YConfiguration args) {
        ...
        eventProducer = EventProducerFactory.getEventProducer(null,
                RFC1055PacketInputStream.class.getName(), 0);
    ...
    public byte[] readPacket() throws IOException {
      return getPayload(dataInputStream);
}
    
    /**
     * This implements the receiving side of RFC 1055.
     * For more information on the standard, go to https://datatracker.ietf.org/doc/html/rfc1055
     * This method is based on the snippet from RFC 1055, Page 5.
     */
