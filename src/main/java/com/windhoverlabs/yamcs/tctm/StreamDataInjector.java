 ...
 * @author nm
 */
public class StreamDataInjector extends AbstractYamcsService
  ...
  public void init(String instance, String name, YConfiguration config) {
    ...
    inDataString = config.getString("data", "");
    inDataLength = inDataString.length() / 2;

    inDataOffset = config.getInt("offset", 0);
    if (inDataOffset < 0) {
      throw new ConfigurationException("Offset must be equal to or greater than 0.");
    }
   ...
   */
  protected byte[] injectData(byte[] pktData) throws IOException {

    ByteArrayOutputStream output = new ByteArrayOutputStream();

    /* Copy the first part of the incoming message to the output stream. If the Offset is zero (inject data at the first byte),
     * this will have no effect.
     */
    output.write(pktData, 0, inDataOffset);

    /* Copy the data we want to inject, to the output stream. */
    output.write(inData, inDataOffset, inDataLength);

    /* Copy the remaining bytes from the incoming message to the output stream.  If the offset was zero, this will copy the
     * entire incoming message.  If the offset is such that it is all the way to the end of the message, this will have no
     * effect.  If the offset is such that it is somewhere inside the message, the initial part of the message was already
     * copied over and this will result in the remainder to be copied over.
     */
    output.write(pktData, inDataOffset, pktData.length - inDataOffset);
  ...
  public void onTuple(Stream arg0, Tuple tuple) {
    if (isRunningAndEnabled()) {
      ...
      byte[] packet;
      try {
        packet = injectData(tuple.getColumn(DATA_CNAME));
