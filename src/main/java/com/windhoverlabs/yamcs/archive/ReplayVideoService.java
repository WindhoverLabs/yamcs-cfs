...
public class ReplayVideoService extends AbstractProcessorService
  ...
  // the modified one sent to the ReplayServer contains the raw data required for
  // extracting/processing those
  // parameters
  ...
  // this can be set in the config (in processor.yaml) to exclude certain parameter groups from
  // replay
  List<String> excludeParameterGroups = null;

  private Parameter spVideoFrameSentCount;
  ...
  public void init(Processor proc, YConfiguration args, Object spec) {
    ...
    if (spec instanceof ReplayOptions) {
      ...
      try {
        ...
        throw new ConfigurationException(
            "Cannot parse config into a replay request: " + e.getMessage(), e);
      }
      if (!rrb.hasSpeed()) {
    ...
    sysParamService = SystemParametersService.getInstance(getYamcsInstance());

    //    spVideoFrameSentCount = sysParamService.createSystemParameter(LINK_NAMESPACE + linkName +
    // "/dataOutCount", Type.UINT64,
    //            "The total number of items (e.g. telecommand packets) that have been sent through
    // this link");
  }
      ...
      case TM_PACKET:
        ...
        if (container == null) {
          ...
          tmProcessor.processPacket(
              new TmPacket(
                  rp.getReceptionTime(),
                  rp.getGenerationTime(),
                  rp.getSequenceNumber(),
                  rp.getPacket()),
              container);
        }

        //            TODO:Send video frames to client(s)
      ...
      case PP:
        ...
        if (!pvals.isEmpty()) {
          ProcessingData processingData =
              ProcessingData.createForTmProcessing(processor.getLastValueCache());
  ...
  private void calibrate(List<ParameterValue> pvlist, ProcessingData processingData) {
    ParameterTypeProcessor ptypeProcessor =
        processor.getProcessorData().getParameterTypeProcessor();
  ...
  // Create rawDataRequest from originalReplayRequest by finding out all raw data (TM and PP)
  // required to provide the
  // needed parameters. The raw request must not contain parameters but only TM or PP.
  //
  // in order to do this, the method addPacketsRequiredForParams will subscribe to all parameters
  // part of the original
  // request, then check in the tmProcessor subscription which containers are needed and in the
  ...
  private void createRawSubscription() throws YamcsException {
    ...
    if (replayAll) {
      ...
      // subscribedParameters set; in case we do not have to retrieve all parameters, create a pp
      // filter such that
    ...
    if (!rawDataRequest.hasPacketRequest() && !rawDataRequest.hasPpRequest()) {
      if (originalReplayRequest.hasParameterRequest()) {
        throw new YamcsException(
            "Cannot find a replay source for any parmeters from request: "
  ...
  private void addPacketsRequiredForParams() throws YamcsException {
    ...
    ParameterWithIdRequestHelper pidrm =
        new ParameterWithIdRequestHelper(
    ...
    try {
      ...
      NamedObjectList nol =
          NamedObjectList.newBuilder().addAllList(e.getInvalidParameters()).build();
    ...
    if ((containers == null) || (containers.isEmpty())) {
      ...
      PacketReplayRequest.Builder rawPacketRequest =
          originalReplayRequest.getPacketRequest().toBuilder();

      for (SequenceContainer sc : containers) {
        rawPacketRequest.addNameFilter(
            NamedObjectId.newBuilder().setName(sc.getQualifiedName()).build());
      }
      log.debug(
          "after TM subscription, the request contains the following packets: "
  ...
  private void createReplay() throws ProcessorException {
    ReplayServer replayServer =
        YamcsServer.getServer().getService(getYamcsInstance(), ReplayServer.class);
  ...
  public void startProviding(Parameter paramDef) {
    // the subscribedParameters is used at the beginning to select the PP parameters which have to
    // be subscribed
  ...
  public ReplayRequest getCurrentReplayRequest() {
    return yarchReplay != null
        ? yarchReplay.getCurrentReplayRequest().toProtobuf()
        : getReplayRequest();
  }
